      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        {trends.map((trend, idx) => (
          <m.div
            ...
            import { m } from "framer-motion";
            import type { CreativePattern, TrendTopic, CompanyProfile } from "@/types/db";
                  ...
                  if (!profile) {
                    ...
                    const complete = Boolean(
                      profile.company_name?.trim() &&
                        ...
                        profile.targeted_keywords.length > 0
                    );
      ...
      <div className="rounded-2xl border-2 border-black/5 bg-white p-4 shadow-card">
        <div className="flex items-center gap-2">
          <div className="rounded-full bg-punch px-3 py-1 text-xs font-semibold text-white shadow-pill">Patterns</div>
                  {profileReady === false && !checkingProfile && (
                    <div className="flex items-center justify-between rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
                      <div className="flex items-center gap-2">
                      ...
                      <Link
                        href="/company"
                        className="rounded-full bg-amber-900 px-3 py-1 text-xs font-semibold text-white shadow-sm"
                      >
                        Go to company page
                      </Link>
                    </div>
