...
export default function AdBuilderPage() {
  ...
  const [result, setResult] = useState<(GenerateAdSuiteResult & { trendsUsed?: string[] }) | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setResult({ strategy: json.strategy, variants: json.variants, trendsUsed: json.trendsUsed });
      setCelebrate(true);
    } catch (err: any) {
    ...
    <div className="mx-auto max-w-6xl space-y-10 pb-20">
      ...
      {result && (
        <div className="space-y-6">
          ...
          {result.trendsUsed && result.trendsUsed.length > 0 && (
            <div className="flex flex-wrap items-center gap-2">
              <span className="text-sm font-black uppercase tracking-wide text-black/60">Trends used:</span>
              {result.trendsUsed.map((t) => (
                <span
                  key={t}
                  className="rounded-full border-2 border-black bg-arcadeGold px-3 py-1 text-xs font-extrabold text-black"
                >
                  {t}
                </span>
              ))}
          ...
          <div className="grid grid-cols-1 gap-8 md:grid-cols-2">
            {result.variants.map((variant) => (
              <AdVariantCard key={variant.variant_index} variant={variant} trendNames={result.trendsUsed} />
            ))}
          </div>
        </div>
      )}
